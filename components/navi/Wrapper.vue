<template>
    <div class="p-5 border-b shadow-md">
            ...
            <client-only >
                <div v-if="!isLoggedIn">
                    <div v-if="!(['/user/login','/user/signup'].indexOf($route.path)>=0)">
                        ...
                        <nuxt-link to="/user/signup" class="transition duration-300 eas-in-out font-sans font-semibold rounded-full px-6 py-2 bg-green-400 text-white text-sm tracking-wide hover:bg-green-600 hover:outline-none focus:outline-none">Sign Up</nuxt-link>
                    </div>
                </div>
                <div v-else>
                    <userProfile />
